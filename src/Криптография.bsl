...
Функция ХАКС(Знач СекретныйКлюч, Знач Сообщение, Знач ХешФункция) Экспорт
	
	ПроверкаПоддержкиХешФункции(ХешФункция);
	
	РазмерБлока = 64;
	
	Если СекретныйКлюч.Размер() > РазмерБлока Тогда
		СекретныйКлюч = Хеш(СекретныйКлюч, ХешФункция);
	КонецЕсли;
...
Функция ДвоичЛев(Знач ДвоичныеДанные, Знач КоличествоБайтов)
	...
	ЧтениеДанных = Новый ЧтениеДанных(ДвочиныеДанные);
